...
class WhisperAM :
    ...
    def transcribe (self, mp3_file) :
        options = dict (language='en', beam_size=self.beam_size, best_of=5)
    ...
    def transcribeFaster (self, mp3_file) :
        options = dict (language='en', beam_size=self.beam_size, best_of=5, vad_filter=True, vad_parameters=dict(min_silence_duration_ms=500))
        transcribe_options = dict (task="transcribe", **options)
    ...
    def getFiles (data_dir) :
        ...
        files3 = glob.glob (data_dir + '/*.MP3')
        files4 = glob.glob (data_dir + '/*.M4A')

        input_data = files1 + files2 + files3 + files4
        return input_data
    ###
